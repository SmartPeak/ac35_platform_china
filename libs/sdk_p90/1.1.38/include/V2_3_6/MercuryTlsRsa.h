...
#ifndef __MERCURY_TLS_RSA_H__
#define __MERCURY_TLS_RSA_H__
 ...
 *                 is not supported.
 */
void mercury_rsa_init( rsa_context *ctx,
               int padding,
               int hash_id,
               int (*f_rng)(void *),
               void *p_rng );
 ...
 */
int mercury_rsa_gen_key( rsa_context *ctx, int nbits, int exponent );
 ...
 * \return         0 if successful, or an XYSSL_ERR_RSA_XXX error code
 */
int mercury_rsa_check_pubkey( rsa_context *ctx );
 ...
 * \return         0 if successful, or an XYSSL_ERR_RSA_XXX error code
 */
int mercury_rsa_check_privkey( rsa_context *ctx );
 ...
 */
int mercury_rsa_public( rsa_context *ctx,
                unsigned char *input,
 ...
 *                 enough (eg. 128 bytes if RSA-1024 is used).
 */
int mercury_rsa_private( rsa_context *ctx,
                 unsigned char *input,
                 unsigned char *output );
 ...
 */
int mercury_rsa_pkcs1_encrypt( rsa_context *ctx,
                       int mode, int  ilen,
                       unsigned char *input,
                       unsigned char *output );
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int mercury_rsa_pkcs1_decrypt( rsa_context *ctx,
                       int mode, int *olen,
                       unsigned char *input,
                       unsigned char *output );
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int mercury_rsa_pkcs1_sign( rsa_context *ctx,
                    int mode,
                    int hash_id,
                    int hashlen,
                    unsigned char *hash,
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int mercury_rsa_pkcs1_verify( rsa_context *ctx,
                      int mode,
                      int hash_id,
                      int hashlen,
                      unsigned char *hash,
 ...
 */
void mercury_rsa_free( rsa_context *ctx );
