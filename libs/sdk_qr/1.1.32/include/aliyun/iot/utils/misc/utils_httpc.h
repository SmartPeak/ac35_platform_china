...
typedef struct {
    ...
    char *url;
    //int url_len;
    char *url_parameter; //fixed buf size HTTP_URL_PARAM_BUF_SIZE, NULL-terminated
    //int url_parameter_len;
    char *header_parameter; //fixed buf size HTTP_HEADER_PARAM_BUF_SIZE, NULL-terminated
    //int header_parameter_len;
    int range_start_pos;
    int range_end_pos;
    char stateCode[4]; //char *stateCode
...
int httpclient_pre_connect(httpclient_t *client, const char *url, int port, const char *ca_crt);
...
int httpclient_get_ssl_last_error(void);
